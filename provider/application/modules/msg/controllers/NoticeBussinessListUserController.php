<?php
class Msg_NoticeBussinessListUserController extends Ec_Controller_Action
{
    public function preDispatch()
    {
        $this->tplDirectory = "msg/views/";
        $this->serviceClass = new Service_NoticeBussinessListUser();
    }
    ...
    public function listAction()
    {
        if ($this->_request->isPost()) {
            ...
            if ($count) {
                ...
                $rows = $this->serviceClass->getByCondition($condition,$showFields, $pageSize, $page, array('nblu_id desc'));
                $return['data'] = $rows;
                $return['state'] = 1;
                $return['message'] = "";
            }
        ...
        echo Ec::renderTpl($this->tplDirectory . "notice_bussiness_list_user_index.tpl", 'layout');
    }
    ...
    public function getByJsonAction()
    {
        ...
        if (!empty($paramId) && $rows = $this->serviceClass->getByField($paramId, 'nblu_id')) {
