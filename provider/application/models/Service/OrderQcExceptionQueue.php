<?php
class Service_OrderQcExceptionQueue extends Common_Service
{
    /**
     * @var null
     */
    ...
    /**
     * @return Table_OrderQcExceptionQueue|null
     */
    public static function getModelInstance()
    {
        if (is_null(self::$_modelClass)) {
            self::$_modelClass = new Table_OrderQcExceptionQueue();
        }
     ...
     * @return mixed
     */
    public static function update($row, $value, $field = "oqeq_id")
    {
        $model = self::getModelInstance();
     ...
     * @return mixed
     */
    public static function delete($value, $field = "oqeq_id")
    {
        $model = self::getModelInstance();
     ...
     * @return mixed
     */
    public static function getByField($value, $field = 'oqeq_id', $colums = "*")
    {
        $model = self::getModelInstance();
    ...
    public  function getFields()
    {
        $row = array(
              ...
              'E8'=>'execute_times',
              'E9'=>'oqeq_status',
              'E10'=>'oqeq_create_time',
              'E11'=>'oqeq_update_time',
        );
        return $row;
    }
    /**
    ...
    public function pickTask($taskNumber){
        ...
        $selections = array('oqeq_id','user_id', 'ute_id', 'erp_no', 'erp_url', 'token','start_time','end_time');
        $condition = array('oqeq_status' => 0, 'execute_times' => 0,'start_time'=>$start_time,'end_time'=>$end_time);
        // 此处排序顺序很重要，不能改
        $taskArray = $model->getByCondition($condition, $selections, $taskNumber, 1, array('execute_times desc'));
        if($taskArray){
            // 查询到有任务记录，锁定
            $taskIds = Common_Common::getArrayColumn($taskArray, 'oqeq_id');
