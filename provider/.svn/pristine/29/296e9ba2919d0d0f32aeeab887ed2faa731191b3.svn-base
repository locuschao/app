<?php
class Table_ProductPictures
{
    protected $_table = null;

    public function __construct()
    {
        $this->_table = new DbTable_ProductPictures();
    }
    ...
    {
        return new Table_ProductPictures();
    }
    ...
    public function getByCondition($condition = array(), $type = '*', $pageSize = 0, $page = 1, $orderBy = "")
    {
        ...
        if(isset($condition["pi_url"]) && $condition["pi_url"] != ""){
            $select->where("pi_url = ?",$condition["pi_url"]);
        }
        /*CONDITION_END*/
        if ('count(*)' == $type) {
