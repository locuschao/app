...
class Common_Barcode
{
    ...
    //生成条码文件
    public static function barcodeDraw($text, $path = '')
    {
        $config = array(
            'barcode' => 'code128',
            'barcodeParams' => array(
        ...
        if (empty($path)) {
            $oldPath = APPLICATION_PATH . '/../data/log/barcode';
            Common_Common::mkdirs($oldPath);
            $path = $oldPath;
        }
        $imageResource = Zend_Barcode::draw(
            'code128', 'image', $config['barcodeParams'], $config['rendererParams']
        );
        $file = $path . '/' . time() . '_' . rand(10, 100) . '.gif';
        imagegif($imageResource, $file, 100);
        return $file;
    }
