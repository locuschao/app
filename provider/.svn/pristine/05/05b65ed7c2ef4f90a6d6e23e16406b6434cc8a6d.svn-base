<?php
class Table_OrderQcExceptionQueue
{
    protected $_table = null;

    public function __construct()
    {
        $this->_table = new DbTable_OrderQcExceptionQueue();
    }
    ...
    {
        return new Table_OrderQcExceptionQueue();
    }
     ...
     * @return mixed
     */
    public function update($row, $value, $field = "oqeq_id")
    {
     ...
     * @return mixed
     */
    public function delete($value, $field = "oqeq_id")
    {
        $where = $this->_table->getAdapter()->quoteInto("{$field}= ?", $value);
     ...
     * @return mixed
     */
    public function getByField($value, $field = 'oqeq_id', $colums = "*")
    {
    ...
    public function getByCondition($condition = array(), $type = '*', $pageSize = 0, $page = 1, $orderBy = "")
    {
        ...
        if(isset($condition["oqeq_status"]) && $condition["oqeq_status"] != ""){
            $select->where("oqeq_status = ?",$condition["oqeq_status"]);
        }
    ...
    public function generateTask() {
        ...
        $sql = "INSERT INTO {$this->_table->info('name')} (user_id,ute_id, erp_no, erp_url, token,start_time,end_time,execute_times,oqeq_status, oqeq_create_time, oqeq_update_time) 
    ...
    public function lockTask($taskId) {
        $date = date('Y-m-d H:i:s');
        $sql = "UPDATE {$this->_table->info('name')} SET oqeq_status = 1, execute_times = execute_times + 1, 
            oqeq_update_time = '{$date}' WHERE oqeq_id IN ($taskId)";
    ...
    {
        $date = date('Y-m-d H:i:s');
        $sql = "UPDATE {$this->_table->info('name')} SET oqeq_status = 0, 
            oqeq_update_time = '{$date}' WHERE oqeq_id IN ($taskId)";
        return $this->_table->getAdapter()->query($sql);
    }
}