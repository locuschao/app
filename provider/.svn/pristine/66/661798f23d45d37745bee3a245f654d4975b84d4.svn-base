<?php
class Common_LoadDataController extends Ec_Controller_Action{
	public function preDispatch()
	{
		$this->tplDirectory = "common/views/";
		$this->serviceClass = new Service_LoadDataControl();
	}
	...
	public function listAction(){
		if($this->_request->isPost()){
			...
			if ($count) {
				...
				$rows = $this->serviceClass->getByCondition($condition,$showFields, $pageSize, $page, array('ldc_id desc'));
				$return['data'] = $rows;
				$return['state'] = 1;
				$return['message'] = "";
			}
			...
			echo Ec::renderTpl($this->tplDirectory . "load_data_index.tpl", 'layout');
		}
